...
Choose whether to enable automatic HTTPS. When enabled, COSY uses [Caddy](https://caddyserver.com/) as its reverse proxy, which automatically obtains and renews TLS certificates from Let's Encrypt.
...
> **Need a domain?** Get a free subdomain at the [Cosy Domain Provider](https://domain-provider.cosy-hosting.net) — claim a `*.play.cosy-hosting.net` subdomain and point it to your server's IP address.
